...
heroes.forEach(hero => {
    ...
    <button onclick="toggleInfo('${hero.name}')">Подробнее</button>
    <div class="info" id="${hero.name}-info">
      <p>${hero.info}</p>
      <div class="stars" id="${hero.name}-stars">
          <span class="star" onmouseover="onStarHover('${hero.name}', 1)" onmouseout="onStarHover('${hero.name}', 0)" onclick="setRating('${hero.name}', 1)">★</span>
          <span class="star" onmouseover="onStarHover('${hero.name}', 2)" onmouseout="onStarHover('${hero.name}', 0)" onclick="setRating('${hero.name}', 2)">★</span>
          <span class="star" onmouseover="onStarHover('${hero.name}', 3)" onmouseout="onStarHover('${hero.name}', 0)" onclick="setRating('${hero.name}', 3)">★</span>
          <span class="star" onmouseover="onStarHover('${hero.name}', 4)" onmouseout="onStarHover('${hero.name}', 0)" onclick="setRating('${hero.name}', 4)">★</span>
          <span class="star" onmouseover="onStarHover('${hero.name}', 5)" onmouseout="onStarHover('${hero.name}', 0)" onclick="setRating('${hero.name}', 5)">★</span>
        </div>
    </div>
  `;
...
function toggleInfo(heroName) {
    const infoDiv = document.getElementById(`${heroName}-info`);
    infoDiv.style.display = infoDiv.style.display === 'none' ? 'block' : 'none';
}

function onStarHover(heroName, rating) {
    const stars = document.querySelectorAll(`#${heroName}-stars .star`);
...
function setRating(heroName, rating) {
    const stars = document.querySelectorAll(`#${heroName}-stars .star`);
    stars.forEach(star => {
        star.style.color = 'black';
    });
    localStorage.setItem(`${heroName}-rating`, rating);
    alert(`Оценка для ${heroName} сохранена: ${rating}`);
}